...
def ventana_gestion_peliculas():
    ...
    pelicula_edit_id = [None]  # Usamos lista para permitir mutación desde funciones anidadas

    def crear_formulario():
        """Crea los campos de entrada para la película."""
    ...
    def cargar_datos_pelicula(pelicula):
        """Carga los datos de una película en el formulario para editar."""
    ...
    def eliminar_y_actualizar(pelicula):
        """Elimina una película y actualiza la lista."""
    ...
    def mostrar_peliculas():
        ...
        for i, peli in enumerate(peliculas):
            ...
            tk.Button(frame_lista, text="Editar", command=lambda p=peli: cargar_datos_pelicula(p)).grid(row=i, column=3)
            tk.Button(frame_lista, text="Eliminar", command=lambda p=peli: eliminar_y_actualizar(p)).grid(row=i, column=4)
    ...
    def crear_botones():
        ...
        tk.Button(btn_frame, text="Guardar película", command=guardar_pelicula).pack(side="left", padx=10)
        tk.Button(btn_frame, text="Regresar", command=ventana.destroy).pack(side="left", padx=10)
